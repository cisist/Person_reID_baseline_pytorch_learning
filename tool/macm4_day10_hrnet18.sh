...
TRAIN_NAME="${TRAIN_NAME:-macm4_day10_hrnet18}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day10}"
...
log "macm4 day-10 HRNet-18 strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running HRNet-18 smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
  python train.py \
    --data_dir "$DATA_DIR" \
    --name "$TRAIN_NAME" \
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal HRNet-18 training on macm4"
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_hr --batchsize "$TRAIN_BATCH" --workers "$WORKERS"
...
log "macm4 day-10 strategy completed"
