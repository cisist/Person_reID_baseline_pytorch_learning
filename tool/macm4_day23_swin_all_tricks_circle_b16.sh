...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day23}"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  set +e
  python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --lr "$LR" --batchsize "$TRAIN_BATCH" --erasing_p 0 --circle --warm_epoch "$WARM_EPOCH" --workers "$WORKERS" --total_epoch 1 2>&1 | tee "$SMOKE_LOG" &
  smoke_pid=$!
...
log "macm4 day-23 strategy completed"
