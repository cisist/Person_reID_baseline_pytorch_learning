...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/jetson_day16}"
...
log "Jetson day-16 strategy started"
...
log "NOTE: DinoV3 is a high-cost model; start with conservative batch sizes on Jetson"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  ...
  timeout "$SMOKE_TIMEOUT" \
    python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_dino --lr "$LR" --batchsize "$TRAIN_BATCH" --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH" \
    2>&1 | tee "$SMOKE_LOG"
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal DinoV3 training"
  run_and_log "$TRAIN_LOG" python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_dino --lr "$LR" --batchsize "$TRAIN_BATCH" --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH"
