...
TRAIN_NAME="${TRAIN_NAME:-macm4_day16_dinov3}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day16}"
...
log "macm4 day-16 DinoV3 strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running DinoV3 smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
  python train.py \
    --data_dir "$DATA_DIR" \
    --name "$TRAIN_NAME" \
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal DinoV3 training on macm4"
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_dino --lr "$LR" --batchsize "$TRAIN_BATCH" --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH" --workers "$WORKERS"
  if [[ "$RUN_TEST_AFTER_TRAIN" == "1" ]]; then
    log "Running test.py after training with which_epoch=$TEST_EPOCH"
    run_and_log "$TEST_LOG" python test.py --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_dino --which_epoch "$TEST_EPOCH" --batchsize "$TEST_BATCH" --workers "$WORKERS"
    if [[ "$RUN_TEST_LAST" == "1" && "$TEST_EPOCH" != "last" ]]; then
      log "Running extra comparison test for which_epoch=last"
      run_and_log "$LAST_TEST_LOG" python test.py --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_dino --which_epoch last --batchsize "$TEST_BATCH" --workers "$WORKERS"
    fi
...
log "macm4 day-16 strategy completed"
