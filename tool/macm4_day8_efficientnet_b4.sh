...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  ...
  if [[ "$RUN_TEST_AFTER_TRAIN" == "1" ]]; then
    ...
    run_and_log "$TEST_LOG" python test.py --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_efficient --which_epoch "$TEST_EPOCH" --batchsize "$TEST_BATCH" --workers "$WORKERS"
    if [[ "$RUN_TEST_LAST" == "1" && "$TEST_EPOCH" != "last" ]]; then
      log "Running extra comparison test for which_epoch=last"
      run_and_log "$LAST_TEST_LOG" python test.py --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_efficient --which_epoch last --batchsize "$TEST_BATCH" --workers "$WORKERS"
    fi
