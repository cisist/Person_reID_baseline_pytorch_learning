...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/jetson_day10}"
...
log "Jetson day-10 strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  ...
  timeout "$SMOKE_TIMEOUT" \
    python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_hr --batchsize "$TRAIN_BATCH" \
    2>&1 | tee "$SMOKE_LOG"
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal HRNet-18 training"
  run_and_log "$TRAIN_LOG" python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_hr --batchsize "$TRAIN_BATCH"
  if [[ "$RUN_TEST_AFTER_TRAIN" == "1" ]]; then
    log "Running test.py after training"
    run_and_log "$TEST_LOG" python test.py --gpu_ids "$GPU_IDS" --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_hr --batchsize "$TEST_BATCH"
