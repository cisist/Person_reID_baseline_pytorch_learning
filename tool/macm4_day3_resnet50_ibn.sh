...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day3}"
...
log "macm4 day-3 ResNet-50-ibn strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running ResNet-50-ibn smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
  python train.py \
    --data_dir "$DATA_DIR" \
    --name "$TRAIN_NAME" \
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal ResNet-50-ibn training on macm4"
...
log "macm4 day-3 strategy completed"
