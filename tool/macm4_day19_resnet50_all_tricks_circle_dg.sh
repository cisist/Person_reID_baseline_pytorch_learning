...
TRAIN_NAME="${TRAIN_NAME:-macm4_day19_resnet50_all_tricks_circle_dg}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day19}"
...
log "macm4 day-19 ResNet-50 all-tricks + circle + DG strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  set +e
  python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --warm_epoch "$WARM_EPOCH" --stride "$STRIDE" --erasing_p 0 --batchsize "$TRAIN_BATCH" --lr "$LR" --circle --DG --workers "$WORKERS" --total_epoch 1 2>&1 | tee "$SMOKE_LOG" &
  smoke_pid=$!
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --warm_epoch "$WARM_EPOCH" --stride "$STRIDE" --erasing_p "$ERASING_P" --batchsize "$TRAIN_BATCH" --lr "$LR" --circle --DG --workers "$WORKERS"
...
log "macm4 day-19 strategy completed"
