...
TRAIN_NAME="${TRAIN_NAME:-macm4_day24_swin_all_tricks_circle_b16_dg}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day24}"
...
log "macm4 day-24 Swin all-tricks + circle + DG strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  set +e
  python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --lr "$LR" --batchsize "$TRAIN_BATCH" --DG --erasing_p 0 --circle --warm_epoch "$WARM_EPOCH" --workers "$WORKERS" --total_epoch 1 2>&1 | tee "$SMOKE_LOG" &
  smoke_pid=$!
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --lr "$LR" --batchsize "$TRAIN_BATCH" --DG --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH" --workers "$WORKERS"
...
log "macm4 day-24 strategy completed"
