...
TRAIN_NAME="${TRAIN_NAME:-macm4_day22_swin_all_tricks_circle}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day22}"
...
log "macm4 day-22 Swin all-tricks + circle strategy started"
log "TRAIN_NAME=$TRAIN_NAME TRAIN_BATCH=$TRAIN_BATCH TEST_BATCH=$TEST_BATCH TEST_EPOCH=$TEST_EPOCH RUN_TEST_LAST=$RUN_TEST_LAST WORKERS=$WORKERS ERASING_P=$ERASING_P WARM_EPOCH=$WARM_EPOCH"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  set +e
  python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --erasing_p 0 --circle --warm_epoch "$WARM_EPOCH" --batchsize "$TRAIN_BATCH" --workers "$WORKERS" --total_epoch 1 2>&1 | tee "$SMOKE_LOG" &
  smoke_pid=$!
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH" --batchsize "$TRAIN_BATCH" --workers "$WORKERS"
...
log "macm4 day-22 strategy completed"
