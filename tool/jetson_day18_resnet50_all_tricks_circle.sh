...
log "Jetson day-18 strict reproduction started"
...
log "README reference: python train.py --warm_epoch 5 --stride 1 --erasing_p 0.5 --batchsize 8 --lr 0.02 --name warm5_s1_b8_lr2_p0.5_circle --circle"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running ResNet-50 (all tricks+Circle) smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal ResNet-50 (all tricks+Circle) training"
...
log "Day-18 strict reproduction completed"
