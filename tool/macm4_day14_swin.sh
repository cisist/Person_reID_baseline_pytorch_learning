...
TRAIN_NAME="${TRAIN_NAME:-macm4_day14_swin}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day14}"
...
log "macm4 day-14 Swin strategy started"
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal Swin training on macm4"
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --batchsize "$TRAIN_BATCH" --workers "$WORKERS"
...
log "macm4 day-14 strategy completed"
