...
TRAIN_NAME="${TRAIN_NAME:-macm4_day4_densenet121}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day4}"
...
log "macm4 day-4 DenseNet-121 strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running DenseNet-121 smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
  python train.py \
    --data_dir "$DATA_DIR" \
    --name "$TRAIN_NAME" \
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal DenseNet-121 training on macm4"
...
log "macm4 day-4 strategy completed"
