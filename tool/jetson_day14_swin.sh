...
log "Jetson day-14 strict reproduction started"
...
log "README reference: python train.py --use_swin --name swin; python test.py --name swin"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running Swin (224x224) smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal Swin (224x224) training"
...
log "Day-14 strict reproduction completed"
