...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/jetson_day14}"
...
log "Jetson day-14 strategy started"
...
log "NOTE: Swin uses 224x224 inputs and is a higher-cost model on Jetson"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running Swin smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
  timeout "$SMOKE_TIMEOUT" \
    python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --batchsize "$TRAIN_BATCH" \
    2>&1 | tee "$SMOKE_LOG"
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal Swin training"
  run_and_log "$TRAIN_LOG" python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --batchsize "$TRAIN_BATCH"
