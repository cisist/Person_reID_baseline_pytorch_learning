...
TRAIN_NAME="${TRAIN_NAME:-macm4_day13_pcb_dg}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day13}"
...
log "macm4 day-13 PCB + DG strategy started"
...
log "NOTE: DG recipe requires DG-Market data to be prepared"
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal PCB + DG training on macm4"
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --PCB --DG --train_all --lr "$LR" --batchsize "$TRAIN_BATCH" --workers "$WORKERS"
...
log "macm4 day-13 strategy completed"
