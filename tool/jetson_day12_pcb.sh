...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/jetson_day12}"
...
log "Jetson day-12 strict reproduction started"
log "ROOT_DIR=$ROOT_DIR"
log "DATA_DIR=$DATA_DIR"
log "TEST_DIR=$TEST_DIR"
log "TRAIN_NAME=$TRAIN_NAME"
log "LR=$LR TEST_EPOCHS=$TEST_EPOCHS GPU_IDS=$GPU_IDS"
log "RUN_SMOKE=$RUN_SMOKE RUN_FORMAL_TRAIN=$RUN_FORMAL_TRAIN RUN_TEST_AFTER_TRAIN=$RUN_TEST_AFTER_TRAIN"
log "README reference: python train.py --name PCB --PCB --train_all --lr 0.02"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running PCB smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal PCB training"
...
log "Day-12 strict reproduction completed"
