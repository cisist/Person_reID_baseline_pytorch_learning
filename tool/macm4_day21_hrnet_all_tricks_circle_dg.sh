...
TRAIN_NAME="${TRAIN_NAME:-macm4_day21_hrnet_all_tricks_circle_dg}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day21}"
...
log "macm4 day-21 HRNet all-tricks + circle + DG strategy started"
...
log "NOTE: DG recipe requires DG-Market data"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  set +e
  python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_hr --lr "$LR" --batchsize "$TRAIN_BATCH" --DG --erasing_p 0 --circle --warm_epoch "$WARM_EPOCH" --workers "$WORKERS" --total_epoch 1 2>&1 | tee "$SMOKE_LOG" &
  smoke_pid=$!
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  run_and_log "$TRAIN_LOG" python train.py --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_hr --lr "$LR" --batchsize "$TRAIN_BATCH" --DG --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH" --workers "$WORKERS"
  if [[ "$RUN_TEST_AFTER_TRAIN" == "1" ]]; then
    run_and_log "$TEST_LOG" python test.py --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_hr --which_epoch "$TEST_EPOCH" --batchsize "$TEST_BATCH" --workers "$WORKERS"
    if [[ "$RUN_TEST_LAST" == "1" && "$TEST_EPOCH" != "last" ]]; then
      run_and_log "$LAST_TEST_LOG" python test.py --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --use_hr --which_epoch last --batchsize "$TEST_BATCH" --workers "$WORKERS"
    fi
...
log "macm4 day-21 strategy completed"
