...
TRAIN_NAME="${TRAIN_NAME:-macm4_day7_resnet50_ibn_usam}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day7}"
...
log "macm4 day-7 ResNet-50-ibn + USAM strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  ...
  if [[ "$smoke_status" -eq 143 ]]; then
    log "Smoke test was stopped after ${SMOKE_TIMEOUT}s; startup path looks healthy"
  else
    log "Smoke test exited cleanly"
  fi
else
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal ResNet-50-ibn + USAM training on macm4"
...
log "macm4 day-7 strategy completed"
