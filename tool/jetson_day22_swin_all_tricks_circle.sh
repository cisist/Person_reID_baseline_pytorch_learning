...
log "Jetson day-22 strict reproduction started"
log "ROOT_DIR=$ROOT_DIR"
log "DATA_DIR=$DATA_DIR"
log "TEST_DIR=$TEST_DIR"
log "TRAIN_NAME=$TRAIN_NAME"
log "ERASING_P=$ERASING_P WARM_EPOCH=$WARM_EPOCH TEST_EPOCHS=$TEST_EPOCHS GPU_IDS=$GPU_IDS"
log "RUN_SMOKE=$RUN_SMOKE RUN_FORMAL_TRAIN=$RUN_FORMAL_TRAIN RUN_TEST_AFTER_TRAIN=$RUN_TEST_AFTER_TRAIN"
log "README reference: python train.py --use_swin --name swin_p0.5_circle_w5 --erasing_p 0.5 --circle --warm_epoch 5; python test.py --name swin_p0.5_circle_w5"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running Swin (all tricks+Circle 224x224) smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal Swin (all tricks+Circle 224x224) training"
...
log "Day-22 strict reproduction completed"
