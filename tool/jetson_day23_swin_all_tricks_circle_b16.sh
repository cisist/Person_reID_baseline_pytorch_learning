...
log "Jetson day-23 strategy started"; log "TRAIN_NAME=$TRAIN_NAME TRAIN_BATCH=$TRAIN_BATCH TEST_BATCH=$TEST_BATCH GPU_IDS=$GPU_IDS LR=$LR ERASING_P=$ERASING_P WARM_EPOCH=$WARM_EPOCH"; [[ -d "$DATA_DIR" ]] || { log "ERROR: DATA_DIR does not exist: $DATA_DIR"; exit 1; }; [[ -d "$TEST_DIR" ]] || { log "ERROR: TEST_DIR does not exist: $TEST_DIR"; exit 1; }
{ python --version || true; nvidia-smi || true; } 2>&1 | tee "$ENV_LOG"
if [[ "$RUN_SMOKE" == "1" ]]; then set +e; timeout "$SMOKE_TIMEOUT" python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --lr "$LR" --batchsize "$TRAIN_BATCH" --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH" 2>&1 | tee "$SMOKE_LOG"; smoke_status=${PIPESTATUS[0]}; set -e; [[ "$smoke_status" -eq 0 || "$smoke_status" -eq 124 ]] || { log "ERROR: smoke failed with $smoke_status"; exit "$smoke_status"; }; fi
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then run_and_log "$TRAIN_LOG" python train.py --gpu_ids "$GPU_IDS" --data_dir "$DATA_DIR" --name "$TRAIN_NAME" --use_swin --lr "$LR" --batchsize "$TRAIN_BATCH" --erasing_p "$ERASING_P" --circle --warm_epoch "$WARM_EPOCH"; if [[ "$RUN_TEST_AFTER_TRAIN" == "1" ]]; then run_and_log "$TEST_LOG" python test.py --gpu_ids "$GPU_IDS" --test_dir "$TEST_DIR" --name "$TRAIN_NAME" --batchsize "$TEST_BATCH"; run_and_log "$EVAL_LOG" python evaluate.py; fi; else log "Formal training not started. To continue today, rerun with RUN_FORMAL_TRAIN=1"; fi
log "Day-23 strategy completed"
