...
log "Jetson day-21 strict reproduction started"
log "ROOT_DIR=$ROOT_DIR"
log "DATA_DIR=$DATA_DIR"
log "TEST_DIR=$TEST_DIR"
log "TRAIN_NAME=$TRAIN_NAME"
log "TRAIN_BATCH=$TRAIN_BATCH LR=$LR ERASING_P=$ERASING_P WARM_EPOCH=$WARM_EPOCH TEST_EPOCHS=$TEST_EPOCHS GPU_IDS=$GPU_IDS"
log "RUN_SMOKE=$RUN_SMOKE RUN_FORMAL_TRAIN=$RUN_FORMAL_TRAIN RUN_TEST_AFTER_TRAIN=$RUN_TEST_AFTER_TRAIN"
log "README reference: python train.py --use_hr --name hr18_p0.5_circle_w5_b16_lr0.01_DG --lr 0.01 --batch 16 --DG --erasing_p 0.5 --circle --warm_epoch 5; python test.py --name hr18_p0.5_circle_w5_b16_lr0.01_DG"
log "NOTE: Strict reproduction mode: numeric defaults follow README. train.py uses --batchsize instead of --batch. Requires DG-Market data."
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running HRNet-18 (all tricks+Circle+DG) smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal HRNet-18 (all tricks+Circle+DG) training"
...
log "Day-21 strict reproduction completed"
