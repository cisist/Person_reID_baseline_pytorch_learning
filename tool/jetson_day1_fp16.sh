...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/jetson_day1}"
STAMP="$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"

ENV_LOG="$LOG_DIR/${STAMP}_env.log"
PREPARE_LOG="$LOG_DIR/${STAMP}_prepare.log"
...
log "Jetson day-1 strategy started"
log "ROOT_DIR=$ROOT_DIR"
log "DATA_DIR=$DATA_DIR"
log "TEST_DIR=$TEST_DIR"
log "TRAIN_NAME=$TRAIN_NAME"
log "TRAIN_BATCH=$TRAIN_BATCH TEST_BATCH=$TEST_BATCH GPU_IDS=$GPU_IDS"
log "RUN_PREPARE=$RUN_PREPARE RUN_SMOKE=$RUN_SMOKE RUN_FORMAL_TRAIN=$RUN_FORMAL_TRAIN RUN_TEST_AFTER_TRAIN=$RUN_TEST_AFTER_TRAIN"
...
log "Collecting environment snapshot"
{
  ...
  python - <<'PY'
import sys
mods = ["torch", "torchvision", "timm", "pretrainedmodels"]
for name in mods:
    try:
        mod = __import__(name)
...
PY
  ...
  find "$DATA_DIR" -maxdepth 2 -type d | sort | head -n 50 || true
} 2>&1 | tee "$ENV_LOG"

if [[ "$RUN_PREPARE" == "1" ]]; then
  log "Running prepare.py"
  run_and_log "$PREPARE_LOG" python prepare.py
else
  log "Skipping prepare.py"
fi

if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running fp16 smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal fp16 training"
