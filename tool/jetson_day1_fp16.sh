...
log "Jetson day-1 strict reproduction started"
...
log "README reference: python train.py --name fp16 --fp16 --train_all"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running ResNet-50 (fp16) smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal ResNet-50 (fp16) training"
...
log "Day-1 strict reproduction completed"
