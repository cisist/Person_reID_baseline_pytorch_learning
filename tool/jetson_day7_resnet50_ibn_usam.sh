...
TRAIN_NAME="${TRAIN_NAME:-ibn-usam}"
...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/jetson_day7}"
...
log "Jetson day-7 strict reproduction started"
...
log "README reference: python train.py --train_all --name ibn-usam --ibn --usam; python test.py --name ibn-usam"
log "NOTE: Training still follows README. Post-train test evaluates net_best.pth and net_last.pth by default; set TEST_EPOCHS to override."
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running ResNet-50-ibn + USAM smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal ResNet-50-ibn + USAM training"
...
log "Day-7 strict reproduction completed"
