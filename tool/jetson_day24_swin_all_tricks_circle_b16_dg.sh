...
log "Jetson day-24 strict reproduction started"
...
log "README reference: python train.py --use_swin --name swin_p0.5_circle_w5_b16_lr0.01_DG --lr 0.01 --batch 16 --DG --erasing_p 0.5 --circle --warm_epoch 5; python test.py --name swin_p0.5_circle_w5_b16_lr0.01_DG"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running Swin (all tricks+Circle+b16+DG 224x224) smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
...
if [[ "$RUN_FORMAL_TRAIN" == "1" ]]; then
  log "Starting formal Swin (all tricks+Circle+b16+DG 224x224) training"
...
log "Day-24 strict reproduction completed"
