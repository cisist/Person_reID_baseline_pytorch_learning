...
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/macm4_day11}"
...
log "macm4 day-11 ResNet-50 + adv strategy started"
...
if [[ "$RUN_SMOKE" == "1" ]]; then
  log "Running ResNet-50 + adv smoke test with timeout=${SMOKE_TIMEOUT}s"
  set +e
  python train.py \
    --data_dir "$DATA_DIR" \
    --name "$TRAIN_NAME" \
...
log "macm4 day-11 strategy completed"
